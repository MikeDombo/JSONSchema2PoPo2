...
class AAAA:
    ...
    def __repr__(self):
        return "<Class AAAA. X: {}, YRef: {}>".format(
            limitedRepr(self.__X[:20] if isinstance(self.__X, bytes) else self.__X),
            limitedRepr(
                self.__YRef[:20] if isinstance(self.__YRef, bytes) else self.__YRef
            ),
        )
